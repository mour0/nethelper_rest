...
fn gen_svg(n: String, r: String, h0: String, h1: String, br: String) -> Result<String, StatusCode> {

    let network_addr = match sanitize_str(&DEFAULT, &n) {
        Ok(res) => res,
        Err(_) => return Err(StatusCode::BAD_REQUEST),
    };

    let router_addr = match sanitize_str(&DEFAULT, &r) {
        Ok(res) => res,
        Err(_) => return Err(StatusCode::BAD_REQUEST),
    };

    let h0_addr = match sanitize_str(&DEFAULT, &h0) {
        Ok(res) => res,
        Err(_) => return Err(StatusCode::BAD_REQUEST),
    };

    let h1_addr = match sanitize_str(&DEFAULT, &h1) {
        Ok(res) => res,
        Err(_) => return Err(StatusCode::BAD_REQUEST),
    };

    let broadcast_addr = match sanitize_str(&DEFAULT, &br) {
        Ok(res) => res,
        Err(_) => return Err(StatusCode::BAD_REQUEST),
    };
...
async fn handler_ipv4(Query(data): Query<NetworkQuery>) -> Result<String,StatusCode> {
    ...
    let generated_svg = gen_svg(data.n, data.r, data.h0, data.h1, data.br)?;
...
async fn handler_save(Query(data): Query<SaveQuery>, db_pool: Extension<SqlitePool>) -> StatusCode {
    ...
    let formatted_template = match gen_svg(data.n, data.r, data.h0, data.h1, data.br) {
        Ok(generated_svg) => generated_svg,
